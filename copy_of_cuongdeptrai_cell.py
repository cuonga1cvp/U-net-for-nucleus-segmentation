# -*- coding: utf-8 -*-
"""Copy of cuongdeptrai_cell.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1KXiLDlAU_O6j-i1j_Cdjaa1w0YNpbhqN
"""
...
class DataGenerator(Sequence):
  ...
  def __getitem__(self, index):
    ...
    images = np.array([cv2.resize(cv2.imread(self.image_paths[k]), self.image_size) for k in indexes])    
		...
		#     images = self.augmentor(images)
    # images = np.array([preprocess_input(img) for img in images])
    return images, np.expand_dims(labels, axis=-1)
...
def loss_sum(y_true, y_pred):
  return 1/2*dice_coef_loss(y_true, y_pred) + Active_Contour_Loss(y_true, y_pred)
...
model.compile(opt1, loss_func, metrics=[dice_coef_loss, Active_Contour_Loss, jac_distance])
